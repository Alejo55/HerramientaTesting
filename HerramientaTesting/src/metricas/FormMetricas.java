...
public class FormMetricas extends JFrame {
	...
	public FormMetricas() {
		...
		splitPane2.setResizeWeight(0.45);
		...
		scrlCom = new JScrollPane();
		scrlCom.setPreferredSize(new Dimension(0, 0));
		splitPane3.setLeftComponent(scrlCom);

		txtCom = new JTextArea();
		txtCom.setFont(new Font("Courier New", Font.PLAIN, 12));
		txtCom.setTabSize(2);
		txtCom.setEditable(false);
		txtCom.setLineWrap(true);
		scrlCom.setViewportView(txtCom);
		
		scrlCodigo = new JScrollPane();
		scrlCodigo.setPreferredSize(new Dimension(0, 0));
		splitPane3.setRightComponent(scrlCodigo);
...
//		tc.setPreferredWidth(60);
	...
	private void mostrarCodigoYRecomendaciones() {
		...
		if (row != -1) {
			...
			txtCom.setText(m.getRecomendaciones());
			txtCom.setCaretPosition(0);
		}
	}
